          ...
          {currentItems.map((item) => (
            <li key={item.id} className="py-4 flex justify-between items-center">
              <div className="flex-grow">
                <Link href={`/koinonia/notice/${item.id}`} className="text-xl font-semibold text-gray-800 hover:text-blue-600 transition-colors duration-300">
                    {item.title}
                </Link>
              ...
              <Link href={`/koinonia/notice/${item.id}`} className="inline-block px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors duration-300 flex-shrink-0 ml-4">
                  보기
                </Link>
            </li>
          ))}
